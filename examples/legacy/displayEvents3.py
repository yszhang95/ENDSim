...
KE_TOLERANCE_MEV = 0.0001 # 1.0 MeV
...
def process_and_visualize_events(ratpac_filename, meta_tree, output_tree, genie_tree):
    ...
    # --- New: Parse event range from RATPAC filename ---
    evt_range = None
    ...
    # Read all RATPAC branches needed
    ...
    for i in range(num_events):
        ...
        # --- New: Loop through primary particles to find the muon ---
        muon_found = False
        for p_idx, pdg in enumerate(output_data["mcpdgs"][i]):
            if pdg in [PDG_MUON, PDG_ANTI_MUON]:
                muon_ke = output_data["mckes"][i][p_idx]
                muon_dir = np.array([
                    output_data["mcus"][i][p_idx],
                    output_data["mcvs"][i][p_idx],
                    output_data["mcws"][i][p_idx]
                ])
                muon_pos = np.array([
                    output_data["mcxs"][i][p_idx],
                    output_data["mcys"][i][p_idx],
                    output_data["mczs"][i][p_idx]
                ])
                muon_found = True
                print(f"  Found muon (PDG={pdg}) at particle index {p_idx}.")
                break # Process the first muon found in the event
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--ratpac', required=True, help="Path to the RATPAC output file (e.g., '...display_21700-21800.root').")
